...
module.exports = class Loader extends Nanoresource {
  ...
  async _loadKeys (namespace, opts) {
    await this.open()
    const keys = this._generateKeys(namespace, opts)
    await this.db.saveKeys(keys, NAMESPACE_SEPARATOR)
    if (!keys.name) keys.name = await this.db.getName(keys)
    if (keys.secretKey) return keys
    return this._generateKeys(namespace, {
      ...opts,
      name: keys.name,
      fullName: keys.name
    })
  ...
  _createStorage (storage, keysProm) {
    ...
    const storageProm = keysProm.then(keys => {
      const id = this._getCacheId(keys)
  ...
  _create (id, namespace, opts, keys) {
    ...
    const keysProm = this._loadKeys(namespace, opts, keys)
    const core = new Omega(this._createStorage(this.storage, keysProm), publicKey, {
      ...this.opts,
      ...opts,
      cache: cacheOpts,
